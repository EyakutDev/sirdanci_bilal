...
// --- Bileşenler... Kodun büyük bir kısmı aynı kaldı ---
const FloatingCallButton = () => { const phoneNumber = "+902329997001"; return (<a href={`tel:${phoneNumber}`} className="floating-call-btn" data-tooltip="Bizi Arayın!"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg></a>);};
const DeliveryPlatforms = () => { return (<section id="paket-servis" className="section"><h2 className="section-title">Paket Servis</h2><div className="delivery-platforms-container"><a href="#" target="_blank" rel="noopener noreferrer" className="platform-card"><img src="/trendyol-logo.png" alt="Trendyol Go" /><span>Trendyol Go'dan Sipariş Ver</span></a><a href="#" target="_blank" rel="noopener noreferrer" className="platform-card"><img src="/yemeksepeti-logo.png" alt="Yemeksepeti" /><span>Yemeksepeti'nden Sipariş Ver</span></a></div></section>);};

// --- TÜM CSS STİLLERİ (İlgili yerler güncellendi) ---
const GlobalStyles = () => {
    const allCSS = `
    /* ... Önceki CSS kodlarının çoğu aynı kaldı ... */
    ...
    .app-header { z-index: 1000; /* ...diğer stiller... */ }
    ...
    @media (max-width: 768px) {
      ...
      /* --- YENİ EKLENEN MOBİL GARANTİ KODU --- */
      .floating-call-btn {
        width: 55px;   /* Mobil için ikonu çok az küçülttük */
        height: 55px;
        bottom: 20px;  /* Telefonların alt barına değmemesi için biraz yukarı aldık */
        right: 20px;
      }
      .floating-call-btn::before {
        display: none; /* Mobilde üzerine gelme yazısı kafa karıştırmasın */
      }
    }
  `;
...
//--- Diğer tüm bileşenler (Header, Hero, Menu, vs.) aynı kaldı ---
const Header = () => {const [isMenuOpen, setIsMenuOpen] = useState(false);const toggleMenu = () => { setIsMenuOpen(!isMenuOpen); };const closeMenu = () => { setIsMenuOpen(false); };return (<header className="app-header"><div className="header-container"><Link to="hero" smooth={true} duration={750} className="logo" onClick={closeMenu}>Şırdancı Bilal</Link><button className="hamburger-menu" onClick={toggleMenu}><span /><span /><span /></button><nav className={`nav-links ${isMenuOpen ? 'open' : ''}`}><Link to="about" smooth={true} duration={750} className="sc-link" onClick={closeMenu}>Şırdan Nedir?</Link><Link to="menu" smooth={true} duration={750} className="sc-link" onClick={closeMenu}>Ürünlerimiz</Link><Link to="paket-servis" smooth={true} duration={750} className="sc-link" onClick={closeMenu}>Paket Servis</Link><Link to="contact" smooth={true} duration={750} className="sc-link" onClick={closeMenu}>İletişim</Link></nav></div></header>);};
const Hero = () => { const imageFiles = [ 'hero-1.jpg', 'hero-2.jpg', 'hero-3.jpg', ]; return ( <section id="hero" className="carousel-container"><Carousel autoPlay={true} infiniteLoop={true} interval={5000} showThumbs={false} showStatus={false} showArrows={false} transitionTime={1000} > {imageFiles.map((fileName, index) => ( <div key={index}><img src={`/${fileName}`} alt={`Şırdancı Bilal Arkaplan ${index + 1}`} /></div> ))} </Carousel><div className="hero-content"><h1>ŞIRDANCI BİLAL</h1><p className="subtitle">Adana'dan Gelen Efsane Lezzetler Şimdi İzmir'de!</p><Link to="menu" smooth={true} duration={750} className="cta-button">Menüyü Gör</Link></div></section> );};
